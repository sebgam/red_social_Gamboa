 ...
 * @version 		$Id: link.class.php 5125 2013-01-14 08:47:27Z Miguel_Espinoza $
 */
class Like_Component_Block_Link extends Phpfox_Component
{
	...
	public function process()
	{
		...
		else if ($sModule == 'profile' && $this->getParam('like_type_id') == 'feed_comment')
		{
		    $sModule = 'feed';
		}
		...
		if (Phpfox::getParam('like.allow_dislike') && Phpfox::hasCallback($sModule, 'getActions'))
		{
			...
			if (!empty($aActions))
			{
				...
				foreach ($aActions as $sName => $aRow)
				{
					...
					$bCheck1 = ($sItemTypeId != 'profile' && $aRow['item_type_id'] != $sItemTypeId && $sItemTypeId != 'core');
					$bCheck2 = (!empty($mReq1)) && (!isset($aRow['where_to_show']) || !is_array($aRow['where_to_show']) || !in_array($mReq1, $aRow['where_to_show']));
