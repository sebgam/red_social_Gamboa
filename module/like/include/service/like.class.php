 ...
 * @version 		$Id: like.class.php 5102 2013-01-08 10:57:36Z Miguel_Espinoza $
 */
class Like_Service_Like extends Phpfox_Service 
{
	...
	public function getActionsFor($sItemTypeId, $iItemId)
	{
		...
		{
			if ($aAction['item_type_id'] == $sItemTypeId || ($aModule[0] . '-' . $aAction['item_type_id']) == $sItemTypeId || ($aModule[0] == $aAction['item_type_id']))
			{
				$aThisAction = $aAction;
				break;
			}
		...
		$aActions = $this->database()->select('a.*, u.user_name, u.full_name, f.friend_id as is_friend')
		...
		foreach ($aActions as $aAction)
		{
			...
			if ($aAction['is_friend'] > 0)
			{
				...
				{
					// find the user id				
					$aOut[$aAction['action_type_id']]['friends'][$aAction['user_id']] = array('user_name' => $aAction['user_name'], 'full_name' => $aAction['full_name']);
