 ...
 * @author  		Raymond_Benc
 * @package 		Phpfox
 * @version 		$Id: link.html.php 5100 2013-01-08 09:29:05Z Miguel_Espinoza $
 */
 ...
 if (Phpfox ::getParam('like.allow_dislike') && isset ( $this->_aVars['aActions'] )): ?>
		...
		<script type="text/javascript">
			...
			$Behavior.assertDislikesDiv = function()
			{
...
<li class="li_action">
	<a href="#" onclick="if ( $(this).parent().find('.like_action_unmarked').is(':visible')){ $(this).parent().find('.like.action.unmarked').click(); }else{}$(this).parents('div:first').find('.js_like_link_unlike:first').show(); $(this).hide(); $.ajaxCall('like.add', 'type_id=<?php echo $this->_aVars['aLike']['like_type_id']; ?>&amp;item_id=<?php echo $this->_aVars['aLike']['like_item_id']; ?>&amp;parent_id=<?php if (isset ( $this->_aVars['aFeed']['feed_id'] )):  echo $this->_aVars['aFeed']['feed_id'];  else:  endif;  if ($this->_aVars['aLike']['like_is_custom']): ?>&amp;custom_inline=1<?php endif; ?>', 'GET'); return false;" class="js_like_link_like"<?php if ($this->_aVars['aLike']['like_is_liked']): ?> style="display:none;"<?php endif; ?>><?php echo Phpfox::getPhrase('feed.like'); ?></a>
...
<?php if (isset ( $this->_aVars['aAction']['action_type_id'] )): ?>
	&middot;
		...
		<a href="#" onclick="$Core.Like.Actions.removeAction('<?php echo $this->_aVars['aAction']['action_type_id']; ?>', '<?php echo $this->_aVars['aAction']['item_type_id']; ?>', <?php echo $this->_aVars['aAction']['item_id']; ?>, '<?php echo $this->_aVars['aAction']['module_name']; ?>', <?php echo $this->_aVars['aAction']['iActionIteration']; ?>,  <?php echo $this->_aVars['aFeed']['feed_id']; ?>, this); return false;" class="like_action_<?php echo $this->_aVars['aAction']['action_type_id']; ?>_<?php echo $this->_aVars['aAction']['iActionIteration']; ?> like_action like_action_unmarked" <?php if (! $this->_aVars['aAction']['is_marked']): ?>style="display:none;"<?php endif; ?>>
			&nbsp;Remove <?php echo $this->_aVars['aAction']['phrase']; ?>
		</a>
<?php endif; ?>
